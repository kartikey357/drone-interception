    ...
    def _loop(self):
        ...
        if self.streaming_count < 100:
            ...
            if USE_CAMERA_PERCEPTION:
                ...
                # NOTE: if the marker is out of frame / occluded / too far,
                # camera_obs_is_new simply stays False this tick -- the model
                # still gets predict()'d below (dead-reckoning through the
                # gap), it just doesn't get an update() until the next lock.
            else:
            ...
            if self.model is not None:
                ...
                twist.angular.z = self._yaw_rate_command(target_pos=None)  # search-scan
                self.vel_pub.publish(twist)
